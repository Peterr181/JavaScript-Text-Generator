...
const loremContainer = document.querySelector(".lorem__container");
...
form.addEventListener("submit", function (e) {
  e.preventDefault();
  loremContainer.classList.remove("hidden");
  const value = parseInt(amount.value);

  if (isNaN(value) || value < 0 || value > 10) {
    loremContainer.innerHTML = `${text[0]}`;
  } else {
    let temporaryText = text.slice(0, value);

    temporaryText.map((myText) => {
      loremContainer.innerHTML += `${temporaryText}`;
    });
    copyBtn.addEventListener("click", function () {
      navigator.clipboard.writeText(temporaryText);
...
resetBtn.addEventListener("click", function () {
  loremContainer.classList.add("hidden");
